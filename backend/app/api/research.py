...
_FIELD_MAP: dict[str, str] = {
    ...
    "return_addition": "return_area_per_apt",
    "returns": "returns_percent",
    # Cost — alternate names
    "construction_residential_per_sqm": "cost_per_sqm_residential",
    "residential_per_sqm": "cost_per_sqm_residential",
    "construction_service_per_sqm": "cost_per_sqm_service",
    "service_per_sqm": "cost_per_sqm_service",
    "construction_commercial_per_sqm": "cost_per_sqm_commercial",
    "commercial_per_sqm_cost": "cost_per_sqm_commercial",
    "construction_balcony_per_sqm": "cost_per_sqm_balcony",
    "balcony_per_sqm": "cost_per_sqm_balcony",
    "construction_development_per_sqm": "cost_per_sqm_development",
    "development_per_sqm": "cost_per_sqm_development",
    "construction_parking_per_sqm": "parking_construction",
    "parking_per_sqm": "parking_construction",
    "index_linkage": "cpi_linkage_pct",
    "interest_rate": "financing_interest_rate",
    "financing_rate": "financing_interest_rate",
    "duration_months": "construction_duration_months",
    # Revenue — alternate names
    "price_per_parking": "price_per_sqm_parking",
    "parking_price": "price_per_sqm_parking",
    "parking_per_spot": "price_per_sqm_parking",
    "storage_per_sqm": "price_per_sqm_storage",
    "marketing_discount_percent": "marketing_discount_pct",
    "marketing_discount": "marketing_discount_pct",
    "sales_pace": "sales_pace_per_month",
}


def _map_field_names(data: dict, section: str = "") -> dict:
    ...
    for k, v in data.items():
        ...
        if db_col != k:
            remapped_keys.append(f"{k} → {db_col}")
        mapped[db_col] = v
    if remapped_keys:
        logger.debug(f"Field mapping [{section}]: remapped {len(remapped_keys)} keys: {remapped_keys}")
    return mapped
...
def get_research(project_id: UUID, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "live_data_sources": data.get("live_data_sources"),
    }
...
def preview_research(
    ...
    sections = {
        "planning_parameters": _map_field_names(research.get("planning_parameters", {}), "planning"),
        "cost_parameters": _map_field_names(research.get("cost_parameters", {}), "cost"),
        "revenue_parameters": _map_field_names(research.get("revenue_parameters", {}), "revenue"),
    }
    ...
    return {
        ...
        "live_data_sources": research.get("live_data_sources", {}),
...
def apply_research_to_simulation(
    ...
    planning_data = _map_field_names(research.get("planning_parameters", {}), "planning")
    cost_data = _map_field_names(research.get("cost_parameters", {}), "cost")
    revenue_data = _map_field_names(research.get("revenue_parameters", {}), "revenue")
    logger.info(f"Apply research: planning={len(planning_data)} fields, "
                f"cost={len(cost_data)} fields, revenue={len(revenue_data)} fields")
    ...
    # Extract per-field provenance from research agent output
    field_provenance = research.get("_field_provenance", {})
        ...
        if existing:
            ...
            for k, v in planning_data.items():
                if v is not None:
                    defaults[k] = v
                    counts["planning"] += 1
            pp_prov = {k: v for k, v in field_provenance.items() if k in planning_data}
            defaults["ai_extraction_metadata"] = {
                "source": "market_research_agent",
                "field_provenance": pp_prov,
            }
        ...
        if existing:
            ...
            for k, v in cost_data.items():
                if v is not None:
                    clean[k] = v
                    counts["costs"] += 1
            cp_prov = {k: v for k, v in field_provenance.items() if k in cost_data}
            clean["ai_extraction_metadata"] = {
                "source": "market_research_agent",
                "field_provenance": cp_prov,
            }
        ...
        if existing:
            ...
            for k, v in revenue_data.items():
                if v is not None:
                    clean[k] = v
                    counts["revenue"] += 1
            rp_prov = {k: v for k, v in field_provenance.items() if k in revenue_data}
            clean["ai_extraction_metadata"] = {
                "source": "market_research_agent",
                "field_provenance": rp_prov,
            }
