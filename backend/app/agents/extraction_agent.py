...
TABU_SYSTEM_PROMPT = """You are an expert at extracting COMPREHENSIVE data from Israeli Tabu (נסח טאבו) land registry documents.

IMPORTANT: Extract ALL data from the document — every sub-parcel, every owner, every mortgage, every warning, every lien.

## What to extract:

### Header / Property Info
- block (גוש), parcel (חלקה), sub_parcel (תת-חלקה if single sub-parcel doc)
- city (עיר), registration_office (לשכת רישום מקרקעין)
- nesach_number (מספר נסח), nesach_date (תאריך הנסח — format YYYY-MM-DD)
- area_sqm (שטח הרכוש המשותף במ"ר), shared_area_sqm
- total_sub_parcels (מספר תתי חלקות)
- takanon ("מצוי" or "מוסכם"), shtar_yotzer (שטר יוצר), tik_yotzer (תיק יוצר)
- address (כתובת)

### Sub-Parcels (תתי חלקות) — Extract EVERY sub-parcel
For each sub-parcel extract:
- number (מספר תת-חלקה)
- floor (שם הקומה: "ראשונה", "שניה", etc.), floor_number (numeric: 1, 2, 3)
- area_sqm (שטח תת-חלקה במ"ר)
- shared_ratio (חלק ברכוש המשותף: "42/548"), shared_ratio_decimal
- owners: list of {name, id_number, ownership_type ("מכר"/"ירושה"/"צוואה"), share ("בשלמות"/"1/2"), share_decimal, deed_number}
- mortgages: list of {lender, lender_id, lender_type, share, rank ("ראשונה"/"שנייה"), deed_number}
- warnings: list of {type ("הערת אזהרה"/"הערה על צורך בהסכמה"), regulation ("סעיף 128"/"תקנה 27"), person, person_id, notes, reference}

### General Liens (הערות כלליות) — Extract ALL
For each lien:
- type ("הערה על יעוד מקרקעין", "עיקול", etc.)
- regulation ("תקנה 27", "סעיף 128")
- authority (e.g., "הועדה המקומית לתכנון ולבניה")
- reference (מספר אסמכתא)
- notes (פרטים)
- is_conservation: TRUE if text contains "שימור" or "מבנה לשימור"

### Backward-Compatible Flat Lists
Also populate: owners (flat list of dicts), liens (flat list of dicts), mortgages (flat list of dicts), warnings (flat list of strings)

### Computed Summary
Calculate from the extracted sub-parcels:
- total_apartments, total_registered_area_sqm (sum of sub-parcel areas)
- shared_area_sqm, blue_line_area_sqm (= shared area from header)
- number_of_floors (from highest floor_number), floors (list of floor names)
- apartments_per_floor (dict: {"1": count, "2": count})
- unique_owners_count, owners_with_mortgages, owners_with_warnings
- has_conservation (true if any lien has is_conservation=true)
- conservation_plan (plan number if found)
- average_apartment_size_sqm, smallest_apartment_sqm, largest_apartment_sqm

Return structured JSON matching the schema provided. Do NOT skip any sub-parcels or owners."""
...
def _extract_tabu(db, doc: Document, pdf_text: str, project_id: str) -> None:
    ...
    messages = [
        ...
        HumanMessage(content=f"Extract ALL property data from this Tabu document. Process every sub-parcel, every owner, every mortgage, every warning:\n\n{pdf_text[:50000]}"),
    ]
