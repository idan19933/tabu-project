/**
 * @module scenario-analysis.service
 * @description Agent 4: Alternatives Agent — generates Conservative/Base/Optimistic financial
 * scenarios by applying cost/price multipliers to the base parameters, then asks Claude for
 * actionable optimization recommendations across the three scenarios.
 */
...
/**
 * Runs a full financial calculation for a given set of simulation parameters
 * and returns the key economic KPIs rounded to two decimal places.
 *
 * @param params - Typed simulation parameter object used by the calculation engine.
 * @returns Object with `{ profit, profitability_rate, irr, npv, total_revenue, total_costs }`.
 */
...
/**
 * Generates three financial scenarios (Conservative, Base, Optimistic) for the given simulation
 * and requests AI-generated optimization recommendations based on the scenario spread.
 *
 * Scenario assumptions:
 * - Conservative: all cost categories +10%, all sale prices -5%, sales pace -20%.
 * - Base: unchanged parameters.
 * - Optimistic: all cost categories -5%, all sale prices +10%, sales pace +20%.
 *
 * AI optimizations are generated by Claude at temperature 0.3 and silently skipped if
 * parsing fails, so the function never throws on AI failure.
 *
 * @param sim - Full simulation record including all parameter relations.
 * @returns Object with `{ scenarios, optimizations, analysis_summary }`.
 *          Returns `{ scenarios: [], optimizations: [], error }` on unhandled errors.
 */
